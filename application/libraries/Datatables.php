  ...
  class Datatables
  {
    ...
    */
    protected function produce_output($charset)
    {
      ...
      $sqlQuery = $this->ci->db->last_query();
      $iTotal = $this->get_total_results(FALSE,$sqlQuery);
      $iFilteredTotal = $this->get_total_results(TRUE,$sqlQuery);
    ...
    * @return integer
    */
    protected function get_total_results($filtering = FALSE,$sql = FALSE)
    {
		
		if($sql)
		{
			$sqlArr = explode("LIMIT",$sql);
			$query = $this->ci->db->query("SELECT count(*) as count FROM (".$sqlArr[0].") as abcdef");		
			$result = $query->result();
