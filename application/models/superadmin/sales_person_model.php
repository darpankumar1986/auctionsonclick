...
class Sales_person_model extends CI_Model {
	...
	public function GetTotalRecord() {	
		...
		$query = "SELECT count(*) as total FROM tblmst_sales_person as u LEFT JOIN tbl_state as state ON state.id = u.state_id WHERE 1  ";
	...
	public function GetRecords($start=0, $limit=10) {
		...
		$this->db->join('tbl_state as state','state.id = u.state_id','left');
		$this->db->order_by("u.id", "desc");
		$this->db->limit($limit,$start);
		
        $query = $this->db->get("tblmst_sales_person as u");	*/
		...
		// start from a basic query, which we will expand later
		$query = "SELECT u.* FROM tblmst_sales_person as u LEFT JOIN tbl_state as state ON state.id = u.state_id WHERE 1  ";
  ...
  function checkcityAttributeExist($cityID)
  {
	...
	public function save_data($image=null) {
		...
		{			
			$this->db->where('id', $id);
			$this->db->update('tblmst_sales_person', $data); 
		}
		else
		{
			$data['indate']=date('Y-m-d H:i:s');
			$this->db->insert('tblmst_sales_person',$data); 
		}
		return true;
	}
