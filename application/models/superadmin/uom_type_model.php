<?php
class Uom_type_model extends CI_Model {
	...
	public function GetTotalRecord() {	
		...
		$columns = array("u.uom_id", "u.uom_name");
		$columnCount = count($columns);

		// start from a basic query, which we will expand later
		$query = "SELECT count(*) as total FROM tblmst_uom_type as u WHERE 1  ";
	...
	public function GetRecords($start=0, $limit=10) {
		...
		// start from a basic query, which we will expand later
		$query = "SELECT u.* FROM tblmst_uom_type as u WHERE 1  ";
  ...
  public function GetRecordById($id) {
	...
	public function save_data($image=null) {
		...
		{			
			...
			$this->db->update('tblmst_uom_type', $data); 
		}
		else
		{
			$data['date_created'] = date('Y-m-d H:i:s');			
			$this->db->insert('tblmst_uom_type',$data); 
		}
		return true;
	}
