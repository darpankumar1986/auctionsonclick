<?php
class Bidder_document_model extends CI_Model {
	...
	public function GetTotalRecord() {	
		...
		$query = "SELECT count(*) as total FROM tbl_doc_master as u WHERE 1  ";
	...
	public function GetRecords($start=0, $limit=10) {
		...
		// start from a basic query, which we will expand later
		$query = "SELECT u.* FROM tbl_doc_master as u WHERE 1  ";
  ...
  public function GetRecordById($id) {
	...
	public function save_data($image=null) {
		...
		{			
			...
			$this->db->where('id', $id);
			$this->db->update('tbl_doc_master', $data); 
		}
		else
		{
			$data['date_created'] = date('Y-m-d H:i:s');			
			$this->db->insert('tbl_doc_master',$data); 
		}
		return true;
	}
