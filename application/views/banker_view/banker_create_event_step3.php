...
<?php
    # Start: For Testing Azizur 
    //$this->load->library('Email_new', 'email');
    //$email = new email_new();
    //$mail = $email->sendMailForTesting(1);
    # End: For Testing Azizur


$checkApprover = GetTitleByField('tbl_user_department', "department_id='".$this->session->userdata('depart_id')."' and status=1 and role_id=2", 'role_id');
//$checkApprover = '';
			  ...
			  <div class="dashboard-wrapper">
			   <?php /* ?>
					  ...
					  <div class="container">
						...
						<div class="secttion-right">
						  <div class="table-wrapper btmrg20">
                                                      <?php if($checkApprover <= 0 || $checkApprover == ''){ ?>
                                                      <div style="color: red;">
                                                          
                                                          Presently there is no approver for your department in the system.
                                                      </div>
                                                      <?php } ?>
				...
				<?php 
					if($utype =='approver') 
					{
						$formAction = '/buyer/saveapproverejecteventdata/'.$auctionID;
					} 
					else 
					{
						$formAction = '/buyer/saveeventdata/'.$auctionID;
		...
		<!--show error popup -->
					<div class="form box-content2">
						...
						<div class="row">
							...
							<label>DRT
									<input type="radio" class="radio_icon" name="RadioGroup1" value="DRT" id="RadioGroup1_0">
							</label>
							<label>SARFAESI
									<input type="radio" class="radio_icon" name="RadioGroup1" value="SARFAESI" id="RadioGroup1_1">
							</label>
							<div class="tooltips">
								<img src="<?php echo base_url(); ?>/images/help.png" class="tooltip_icon">
								<span>An event can be initiated by DRT or the event can be initiated by any bank under SARFAESI act. The bank eauctions platform is made for both the events. Please select the option accordingly.</span>
							</div>
							</div>
						</div>
						<?php */ ?>
					...
					<!--  <div class="row">
					 ...
					 -->
					  <div class="row">
						<div class="lft_heading">Property Address <span class="red"> *</span></div>
						<div class="rgt_detail">
					  ...
					  <div class="row">
						...
						<div class="rgt_detail">
							...
							<?php									
								foreach($uomType as $ut){?>
									<!-- <option value="<?php echo $ut->uom_id; ?>" <?php echo ($ut->uom_id==$auctionData->area_unit_id)?'selected':''; ?>><?php echo $ut->uom_name; ?></option> -->
					   ...
					   <div class="row">
							<div class="lft_heading">Category/ Property Type <span class="red"> *</span></div>
							<div class="rgt_detail">
				  ...
				  <div class="row">
					  ...
					  <div class="plain row">
						...
						<div class="rgt_detail">
						 ...
						 <?php } ?>
						  <!--<label>Yes
						  <input name="dsc_enabled" <?php if($auctionData->dsc_enabled==1) { echo $chk='checked';}else{echo $chk='';}?>  id="dsc_enabled" class="radio_icon" type="radio" value="1">
						  </label>
						  <label>No
						  <input name="dsc_enabled" checked <?php if($auctionData->dsc_enabled==0) { echo $chk='checked';}else{echo $chk='';}?>  class="radio_icon" id="dsc_enabled1" type="radio" value="0">
						  </label>-->
						   <div class="tooltips">
							<img src="<?php echo base_url(); ?>/images/help.png" class="tooltip_icon">
...
?>	
					  ...
					  <div class="row" id="div_second_opener">
						...
						*/
						 ?>
						<?php if($utype =='approver') {?>	
						<div class="row">
							<div class="lft_heading">Auction Approvers Comments <span class="red">*</span></div>
							<div class="rgt_detail">
								<textarea name="approverComments" id="approverComments" maxlength="500"><?php echo $auctionData->approverComments;?></textarea>
								<div class="tooltips">
									<img src="<?php echo base_url(); ?>/images/help.png" class="tooltip_icon">
									<span>Please Provide Auction Approvers Comments.</span>
								</div>	
							</div>
						 </div>		
					 <?php } ?>
					  ...
					  <div class="seprator btmrg20"></div>
						...
						<div class="button-row row" style="text-align:center;">
						 <a href="/buyer/savedEvents/" style="font-size:0; padding-right:5px;" class="b_submit button_grey">Cancel</a>
			             <!----> 						
						<?php if($utype =='approver') 						
						{?>
							<input name="review" onclick="return validateSubmitform('review');" value="Review" type="submit" class="b_publish button_grey">
							<input name="approve" onclick="return validateSubmitform('approve');" value="Approve" type="submit" class="b_publish button_grey">
							<input name="reject" onclick="return validateSubmitform('reject');" value="Reject" type="submit" class="b_publish button_grey">
						<?php 
						}
						else
						{
							if($auctionData->status==0 || $auctionData->status=='' || $auctionData->status==2) 
							{ ?>
						   ...
						   <?php 
						    ...
						    {
						   ?>							
                                                         <input name="send4Approval" value="Send For Approval" <?php if($checkApprover == 0 || $checkApprover == '') echo 'disabled="disabled"'; ?> onclick="return validateSubmitform('send4Approval');" type="submit" class="b_submit button_grey float-right">
						<?php }
					    }
					  ?>
...
$("#city_id").attr("value","<?=$auctionData->state?>");
    jQuery('#country_id').change(function(){
		...
		if(country_id )
		{
			...
			jQuery('#state_id').load('/buyer/getStateDropDown/'+country_id+'/'+state_id);
		}
...
?>	
	...
	jQuery('#state_id').change(function(){
		...
		{      $("#city_id").attr('disabled',false);
			...
			jQuery('#city_id').load('/buyer/getCityDropDown/'+state_id+'/'+city_id);
		}
                 });  
