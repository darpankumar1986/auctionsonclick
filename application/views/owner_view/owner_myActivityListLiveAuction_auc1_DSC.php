	...
	{ 
		foreach ($auctionData as $adata) {		
			...
			<script>
				...
				$(document).on('change','#bidder_bid_value_<?php echo $adata->id; ?>',function(){	
					var mtxtValue = $("#bidder_bid_value_<?php echo $adata->id; ?>").val();
					var inputValue = $(this).val();
					var hold = numToWords(inputValue);					
					$('#riw_<?php echo $adata->id; ?>').html(hold);
					//input.innerHTML = hold;
					var areaVal = $("#areaVal_<?php echo $adata->id; ?>").val();                            
...
<?php foreach ($auctionData as $aDta) {
	/*
	?>
$(document).on('change','#bidder_bid_value_<?php echo $aDta->id; ?>',function(){
	
	var mtxtValue = $("#bidder_bid_value_<?php echo $aDta->id; ?>").val();
	var inputValue = $(this).val();
	var hold = numToWords(inputValue);
	alert(hold);
	$('#riw_<?php echo $aDta->id; ?>').html(hold);
    //input.innerHTML = hold;
    var areaVal = $("#areaVal_<?php echo $aDta->id; ?>").val();                            
    
	var totalVal = areaVal*mtxtValue;
	$("#total_bid_val_<?php echo $aDta->id; ?>").val(totalVal);
});
<?php */ } ?>
...
<section class="container_12">						
    <div class="container_12"  id="live_auction_data">
		<div>
	       <?php echo $buylive_bidding_auction_list_eauc_DSC;?>
	</div>
</section>
	...
	$(document).ready(function(){
		...
		setInterval(function(){ 
			...
			$('[data-countdown]').each(function() {
				var $this = $(this), finalDate = $(this).data('countdown');
				$this.countdown(finalDate, function(event) {
					$this.html(event.strftime('<span class="col"><span style="padding:8px;background-color:#000;color:#fff;font-size:18px;">%D</span> <label>Day</label></span><span class="colon">:</span> <span class="col"><label></label> <span>%H</span></span><span class="colon">:</span><span class="col"><label></label> <span>%M</span></span><span class="colon">:</span><span class="col"><label></label> <span>%S</span></span>'));
			   });
		 });
	}, 10000); //10000
