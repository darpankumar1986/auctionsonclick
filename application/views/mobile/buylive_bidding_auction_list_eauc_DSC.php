...
<style>
	.inc_dcm_box{width:30px; float:left; }
...
<script>
	jQuery(document).ready(function($){
		 ...
		 $(".iframeColorbox").colorbox({iframe: true, width: "100%", height: "90%"});
		$(".check_dsc").colorbox({
			inline:true, 
			width:"50%",
			onClosed: function () {
    ...
    text-align: center;">Time Remaining -</div> <br> <?php if ($adata->auction_bidding_activity_status == 1) { ?>
            ...
            <?php }
				...
				//(($autobidExist <= 0) || ($getAutocutRow->auto_bid <= $h1price)) && 	
                    ...
                    <div class="table-section"> 
						<table border="1" align="left" cellpadding="2" cellspacing="1" class="mytable mytable1 dataTable" id="big_table" aria-describedby="big_table_info">							
							<tbody role="alert" aria-live="polite" aria-relevant="all">	
								<!--<tr class="odd">
								  <th width="7%" align="left" valign="top" class=""><strong>Sr.No.</strong></th>
								  <th align="left" valign="top" class=""><strong>Property Details</strong></th>
								  <th align="left" valign="top" class=""><strong>Area</strong></th>
								  <th align="left" valign="top" class=""><strong>UOM</strong></th>
								  <th align="left" valign="top" class=""><strong>Price Basis</strong></th>
								  <th align="left" valign="top" class=""><strong>Rate in Rs.</strong></th>
								  <th align="left" valign="top" class=""><strong>Rate in Words</strong></th>
								  <th align="left" valign="top" class=""><strong>Total Value</strong></th>
								  <th align="left" valign="top" class=""><strong>Bid</strong></th>
								</tr>-->							
								<tr class="odd">
									<!--<td><strong>Sr.No.: </strong>1</td>-->
									<td><strong>Property Details: </strong><?php echo $adata->PropertyDescription; ?></td>
									<td><input type="hidden" name="plotareaVal" id="plotareaVal_<?php  echo $adata->id; ?>" value="<?php echo $adata->property_height; ?>"/><strong>Plot Area: </strong> <?php echo ($adata->property_height !='' )?$adata->property_height.' '. $areaUnit:'N/A'; ?></td>
									<td><input type="hidden" name="areaVal" id="areaVal_<?php  echo $adata->id; ?>" value="<?php echo $adata->area; ?>"/><strong>Carpet Area: </strong> <?php echo ($adata->area !='' )?$adata->area.' '. $areaUnit:'N/A'; ?></td>
									<!--<td><strong>UOM: </strong> <?php echo $areaUnit; ?></td>-->
									<?php if(!in_array($adata->id,$auctionNotPriceBasis) && false) {?><td><strong>Price Basis: </strong><?php echo 'Price per '.$pricePerUnit; ?></td><?php } ?>
									<td><strong>Rate in Rs.: </strong> 
										<div class="half no_rgt_border" style="border:none!important;">
											...
											<div class="cut_off"> 
												...
												<span class="decrement1" onclick="increseORdecriseBidValue('decrease', '<?php echo $adata->id; ?>');"></span>
											</div>
												<input type="text" style="margin-top:0;" class="dynamic_price_live" onkeypress="return isNumberKey(event);" value="<?php echo $bidtextval; ?>"  name="bidder_bid_value_<?php echo $adata->id; ?>" id="bidder_bid_value_<?php echo $adata->id; ?>" class="input-sort" readonly >
												<div class="float-right inc_dcm_box" <?php
												if($getAutocutRow->auto_bid > $h1price) { echo 'style=display:none;'; } ?>>
												<span class="increment1" onclick="increseORdecriseBidValue('increase', '<?php echo $adata->id; ?>','',clicks);"></span>
												
											</div>
											<?php if($getAutocutRow->auto_bid > $h1price) { echo '<div style="float: left;font-size: 12px;width: 100%; text-align: left;padding-left: 10px;padding-top: 5px;">'.'Auto Bid: '.$getAutocutRow->auto_bid.'</div>';}?>
										</div>
										<span class="amount_in_words" id="price_in_ruppees_<?php echo $adata->id; ?>"></span>
										<script type="text/javascript">
									...
									<td><strong>Rate in Words: </strong><span id="riw_<?php echo $auctionID;?>" class="riwClass"></span></td>
									<?php if(!in_array($adata->id,$auctionNotPriceBasis) && false) { ?><td><strong>Total Value:</strong><input type="text" name="total_bid_val_<?php echo $adata->id; ?>"  id="total_bid_val_<?php echo $adata->id; ?>" readonly /></td><?php } ?>
									<td> 
										<div class="live_bid_cnt_btn" >
