...
<section class="body_main1">	
		<div class="row">		
			<a href="<?php echo base_url().'superadmin/master_bank/index'?>" class="button_grey"> Bank List</a>
		</div>
		...
		<div class="centercontent">
			...
			<div id="contentwrapper" class="contentwrapper box-content2">
				<div id="validation" class="subcontent">            	
					<form method="post" class="stdform" id="category" name="add_data_view" accept-charset="utf-8" action="/superadmin/master_bank/save/<?php if($id) echo $id;?>" autocomplete="off">	
					...
					<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash();?>" />
						...
						<div class="row">
							...
							<div class="rgt_detail">
								<input maxlength="100" type="text" name="bank_name" id="bank_name" class="longinput html_found" value="<?php echo $bank_name?>" />
								
								<?php if(isset($master_bank_exists)) {?>
							<label class="error" id="cname"><?php echo $master_bank_exists;?> is already in use</label>	
							<?php }?>	
							</div>	
	...
	jQuery("#category").validate({
		rules: {			
			bank_name: {
				...
				remote: {
					url: "/superadmin/master_bank/uniqueMasterbank",
					type: "post",
					data: {
						name: function() {
							return jQuery( "#bank_name" ).val();
							jQuery("lavel").text('');
						},					
						bank_id: function() {
							return jQuery( "#bank_id" ).val();
						}
					}
				}
			}
