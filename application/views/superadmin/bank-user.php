...
<?php // echo $this->session->userdata('aemail'); ?>	
	<section class="container_12">		
		<div class="row">						
			<a href="<?php echo base_url().'superadmin/user/bankeraddedit'?>" class="button_grey">Create Branch User</a>
		</div>
					...
					<tbody>
						<?php $i=0;foreach($records as $data){$i++;?>
							<tr class='gradeA'>
								...
								<!--<td><?php if($data->user_type == 'branch'){echo 'Creator';}else if($data->user_type == 'approver'){echo 'Approver';}else if($data->user_type == 'pVerifier'){echo 'Payment Verifier';} else if($data->user_type == 'viewer'){echo 'Viewer';} else if($data->user_type == 'aConfirmer'){echo 'Auction Confirmer';}  ?></td>-->
								<td><?php echo $data->indate; ?></td>
								<td>
									<a href="/superadmin/user/assigndept/<?php echo $data->id; ?>">Assign Dept</a> | <a href="/superadmin/user/bankeraddedit/<?php echo $data->id; ?>">Edit</a>
								</td>
							</tr>
						<?php }?>	
