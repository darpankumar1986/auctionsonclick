...
<?php } ?>		
<div class="box-head">GST MIS Report of Bank Processing Fee</div>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb display">
					<tbody>
						...
						<tr>
							...
							<a href="<?php echo base_url()?>superadmin/user/gst_mis_report_bank_processing_fee" class="button_grey">Reset</a>
							</td>
						</tr>
					</tbody>
				</table>
			...
			<tbody>
				<?php $i=0;foreach($records as $data){$i++;
					...
					<tr class='gradeA'>
						...
						<td><?php echo $baseAmt= round(($pay_res->mer_amount*100)/118,2); ?></td>
						<td><?php echo '18.00'; ?></td>
						<td><?php echo $pay_res->mer_amount - $baseAmt; ?></td>
						<td><?php echo $pay_res->mer_amount; ?></td>
						<td><?php $payDate = str_replace('/','-',$pay_res->trans_date);
								echo ($pay_res != '' && $pay_res != null) ? $payDate :$data->date_created; ?></td>
						<td><?php echo 'Online Payment'; ?></td>
						<td><?php echo ($pay_res != '' && $pay_res->bank_ref_no != 'null') ? $pay_res->bank_ref_no :'--' ; ?></td>
