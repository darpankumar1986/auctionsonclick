            ...
            <form action="" method="GET"  id="searchfm" name="searchfm" onsubmit="return validateform();">
				<table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb display">
					<tbody>
						...
						<tr>							
							<td style="width:22%;  vertical-align:top; border-top: 1px solid #ddd;">
								<label><strong>Reference No.</strong></label><input type="text" value="<?php echo $ref_number;?>" id="ref_number" name="ref_number">
							</td>
							<td style="width:22%;  vertical-align:top; border-top: 1px solid #ddd;">
							...
							<a href="<?php echo base_url()?>superadmin/user/payment_log" class="button_grey">Reset</a>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="stdtable display">			
			<thead>
			     <tr>
						...
						<th class="head0">Scheme Address</th>
			...
			<tbody>
				<?php $i=0;foreach($records as $data){$i++;
					...
					<tr class='gradeA'>
						...
						<td><?php echo $scheme_code = GetTitleByField('tbl_auction',"id='".$data->auction_id."'",'reference_no'); ?></td>
						<td><?php $scheme_name = GetTitleByField('tbl_auction',"id='".$data->auction_id."'",'scheme_name');
									echo ($scheme_name !='') ? $scheme_name:'N/A';
						 ?></td>
						<td><?php echo $scheme_address = GetTitleByField('tbl_auction',"id='".$data->auction_id."'",'PropertyDescription'); ?></td>
						<td><?php echo $scheme_address = GetTitleByField('tbl_user_registration',"id='".$data->bidder_id."'",'email_id'); ?></td>
						<td><?php echo ($pay_res != '' && $pay_res != null) ? $pay_res->tracking_id :'--'; ?></td>
						<td>
						<?php 
								echo ($pay_res != '' && $pay_res != null) ? $pay_res->payment_mode :'--';
						?>
						</td>
						<td><?php echo ($pay_res->bank_ref_no != '') ? $pay_res->bank_ref_no :'--' ; ?></td>						
						<td><?php echo ($pay_res != '' && $pay_res != null) ? $pay_res->mer_amount :'--'; ?></td>
						<td><?php 
								$payDate = str_replace('/','-',$pay_res->trans_date);
								echo ($pay_res != '' && $pay_res != null) ? $payDate :$data->date_created;
							?></td>
						<td><?php echo ($data->bidder_IP !='')? $data->bidder_IP:'--'; ?></td>
